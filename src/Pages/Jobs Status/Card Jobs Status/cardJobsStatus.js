...
export default function CardJobsStatus({ jobStatusFilter }) {
    ...
    const { isLoading, data, error } = useQuery(['status', jobStatusFilter], async () => {
        ...
        }).then(res => res.data.applications.filter((item) => {
            return item.currentStatus.toLowerCase() === jobStatusFilter.toLowerCase()
        }))
    })
                                ...
                                <div className='col-12 col-md-6 col-lg-6 '>
                                    {
                                        ...
                                        data.length !== 0 ?
                                            data?.map((d, index) => {
                                                            ...
                                                            <div className='company-card' style={{
                                                                ...
                                                                <div className='company-info'>
                                                                    <div className='company-heading'>
                                                                        <p className='company-job-role'>{d?.jobId?.jobTitle}</p>
                                                                        <p className='company-job-posted'> Posted {diffDays} Day ago</p>
                                                                    </div>
                                                                    <div className='company-address'>
                                                                        <div className='row'>
                                                                            <div className='col'><p>{d?.companyId?.companyName}, {d?.jobId?.country} </p></div>
                                                                    ...
                                                                    <div className='company-job-type'>
                                                                        ...
                                                                        <div className='company-job-type-names'>
                                                                            <span>{d?.jobId?.jobType?.charAt(0).toUpperCase() + d?.jobId?.jobType?.slice(1)}</span>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div><br />
                                                ...
                                                <p>No Jobs are there which shows your {jobStatusFilter.charAt(0).toUpperCase() + jobStatusFilter.slice(1)} status</p>
                                            </>
                                    }
                                </div>
                                <div className='col-md-6 col-lg-6'>
                                    {jobDataById ? <JobDescriptionStatus jobDataById={jobDataById} /> :
                                        <JobDescriptionStatus jobDataById={data.length != 0 ? data[0] : ''} />}
